...
inline void push_move(std::vector<Move>& moves, const Board& board, int fromSq, int toSq, int capturedPiece = 0, int promotion = 0, bool isEnPassant = false, bool isCastling = false) {
    Move m;
    m.fromRow = sq_to_row(fromSq);
    m.fromCol = sq_to_col(fromSq);
    m.toRow = sq_to_row(toSq);
    m.toCol = sq_to_col(toSq);
    m.piece = piece_on_square_bb(board, fromSq);
...
void generate_pawn_moves_bb(const Board& board, std::vector<Move>& moves) {
    ...
    while (pawns) {
            ...
            if (empty & toMask) {
                ...
                if (isPromo) {
                    for (int promo : {queen, rook, bishop, knight}) {
                        push_move(moves, board, from, to, 0, promo, false, false);
                    }
                } else {
                    push_move(moves, board, from, to, 0, 0, false, false);
                    bool onStartRank = whiteToMove ? (from >= 8 && from <= 15) : (from >= 48 && from <= 55);
                    if (onStartRank) {
                        int to2 = whiteToMove ? (from + 16) : (from - 16);
                        Bitboard to2Mask = 1ULL << to2;
                        if (empty & to2Mask) {
                            push_move(moves, board, from, to2, 0, 0, false, false);
                        }
                    }
                }
            }
        ...
        while (attacks) {
            ...
            if (isPromo) {
                for (int promo : {queen, rook, bishop, knight}) {
                    push_move(moves, board, from, capSq, captured, promo, false, false);
                }
            } else {
                push_move(moves, board, from, capSq, captured, 0, false, false);
            }
        }
        ...
        if (board.enPassantCol != -1) {
            ...
            if (pawn_attacks[us][from] & (1ULL << epSq)) {
                ...
                push_move(moves, board, from, epSq, captured, 0, true, false);
            }
        }
    }
}
...
void generate_knight_moves_bb(const Board& board, std::vector<Move>& moves) {
    ...
    while (knights) {
        ...
        while (targets) {
            ...
            if (is_king_piece(captured)) continue;
            push_move(moves, board, from, to, captured, 0, false, false);
        }
    }
}
    ...
    if (whiteToMove && from == 4) {
        if (board.whiteCanCastleKingSide) {
            ...
            if ((occ & emptyMask) == 0 &&
                ...
                rookPresent) {
                push_move(moves, board, 4, 6, 0, 0, false, true);
            }
        }
        if (board.whiteCanCastleQueenSide) {
            ...
            if ((occ & emptyMask) == 0 &&
                ...
                rookPresent) {
                push_move(moves, board, 4, 2, 0, 0, false, true);
            }
        }
    }
    ...
    if (!whiteToMove && from == 60) {
        if (board.blackCanCastleKingSide) {
            ...
            if ((occ & emptyMask) == 0 &&
                ...
                rookPresent) {
                push_move(moves, board, 60, 62, 0, 0, false, true);
            }
        }
        if (board.blackCanCastleQueenSide) {
            ...
            if ((occ & emptyMask) == 0 &&
                ...
                rookPresent) {
                push_move(moves, board, 60, 58, 0, 0, false, true);
            }
        }
    }
}
