...
int16_t negamax(Board& board, int depth, int16_t alpha, int16_t beta, int ply, SearchStack* ss, std::vector<Move>& pvLine, std::vector<uint64_t>& positionHistory) {
    ...
    for (int movesSearched = 0; movesSearched < moveCount; ++movesSearched) {
        ...
        // Singular Extensions
        int extension = 0;
        bool trySingular = !(ply == 0) && (ply < depth * 2) && !ss->singularMove && ttHit && (chosenMove == ttMove) && depth >= 8 && (ttEntry.flag != TT_BETA) && (ttEntry.depth >= depth - 3) && abs(ttEntry.score) < MATE_SCORE - MAX_PLY;
        if (trySingular) {
            ...
            int singularMargin = (depth * 5 + (!pvNode ? 10 : 0)) / 8;
            int singularBeta = ttSeScore - singularMargin;
